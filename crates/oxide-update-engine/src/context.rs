...
use crate::{UpdateEngine, errors::ExecutionError};
...
///
/// This is passed into the function registered for a step, and can be
/// used to send progress updates as the function continues execution.
///
/// # Notes
///
...
impl<S: StepSpec> StepContext<S> {
    ...
    /// Sends a report from a nested engine, typically one running on
    /// a remote machine.
    ///
    ...
    pub async fn send_nested_report<S2: StepSpec>(
        ...
        let delta_report = if let Some(id) = report.root_execution_id {
            ...
            // If there's no root execution ID set, report is expected
            // to be empty. However, report is untrusted data so we
            // can't assert on it. Instead, log this.
            if !report.step_events.is_empty() {
                slog::warn!(
                    self.log,
                    "received non-empty report with empty \
                     root execution ID";
                    "report" => ?report,
                );
            }
        ...
        if let Some(delta_report) = delta_report {
            for event in delta_report.step_events {
                match &event.kind {
                    ...
                    StepEventKind::ExecutionAborted {
                        ...
                        res = Err(NestedEngineError::Aborted {
                            component: aborted_step
                                .info
                                .component
                                .clone(),
                            id: aborted_step.info.id.clone(),
                            description: aborted_step
                                .info
                                .description
                                .clone(),
                    ...
                    // Note: we do not care about nested failures or
                    // aborts. That's because the parent step might
                    // have restarted nested engines. Only top-level
                    // failures or aborts matter.
                ...
                self.payload_sender
                    ...
                    .await
                    .expect(
                        "our code always keeps \
                         payload_receiver open",
                    );
            }
            ...
            // Ensure that all reports have been received by the
    ...
    ///
    /// An individual step can generate other steps: these steps are
    /// treated as *nested*, and carry their own progress.
        ...
        S2: StepSpec + 'a,
    {
        ...
        // `receiver`.
        //
        // That approach was abandoned because it had ordering issues,
        // because it wasn't guaranteed that events were received in
        // the order they were processed. For example, consider what
        // happens if:
        //
        // 1. User code sent an event E1 through a child (nested)
        //    StepContext.
        // 2. Then in quick succession, the same code sent an event E2
        //    through self.
        //
        // What users would expect to happen is that E1 is received
        // before E2. However, what actually happened was that:
        //
        // 1. `engine` was driven until the next suspend point. This
        //    caused E2 to be sent.
        // 2. Then, `receiver` was polled. This caused E1 to be
        //    received.
        //
        // So the order of events was reversed.
        //
        // To fix this, we now use a single channel, and send events
        // through it both from the nested engine and from self.
        //
        // An alternative would be to use a oneshot channel as a
        // synchronization tool. However, sharing a channel is easier.
...
///
/// Nested event reports might contain events already seen in prior
/// runs: `NestedEventBuffer` deduplicates those events such that only
...
/// An uninhabited type for oneshot channels, since we only care about
/// them being dropped.
#[derive(Debug)]
...
///
/// This is passed into the function registered to generate a step's
/// metadata.
///
/// # Notes
///
...
///
/// It is important that `StepHandle`s never be awaited outside the
