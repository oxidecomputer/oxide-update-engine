...
use crate::{
    CompletionContext, MetadataContext, StepContext, StepContextPayload,
    StepHandle, errors::ExecutionError,
};
...
pub struct UpdateEngine<'a, S: StepSpec> {
    ...
    // ComponentRegistrar and NewStep at the same time. (This could also
    // be a `RefCell` if a `Send` bound isn't required.)
    //
    // There is an alternative way to do this that doesn't use a mutex
    // but involves no less than three lifetime parameters, which is
    // excessive.
...
impl<'a, S: StepSpec + 'a> UpdateEngine<'a, S> {
    ...
    ///
    /// An abort handle can be used to forcibly cancel update engine
    /// executions.
    ...
    async fn execute_impl(
        ...
        let steps = {
            ...
            // Grab the steps and component counts from within
            // steps_lock, then let steps_lock go. (Without this,
            // clippy warns about steps_lock being held across await
            // points.)
            //
            // There are no concurrency concerns here because `execute`
            // consumes `self`, and is the only piece of code that has
            // access to the mutex (`self.steps` is a `Mutex<T>`, not
            // an `Arc<Mutex<T>>`!)
...
/// Abstraction used to send events to whatever receiver is interested
/// in them.
///
/// # Why is this type so weird?
///
/// `EngineSender` is a wrapper around a cloneable trait object. Why do
/// we need that?
///
/// `SenderImpl` has two implementations:
///
/// 1. `DefaultSender`, which is a wrapper around an
///    `mpsc::Sender<Event<S>>`. This is used when the receiver is user
///    code.
...
///
/// This handle should be awaited to drive and obtain the result of an
/// execution.
#[derive(Debug)]
...
impl<S: StepSpec> ExecutionHandle<'_, S> {
    ...
    /// can be optionally waited against to block until the abort is
    /// processed.
    ///
...
/// A future which can be used to optionally block until an abort
...
struct Steps<'a, S: StepSpec> {
    ...
    // don't want to impose a `Hash` or `Ord` restriction on
    // `S::Component`. In particular, we want to support
    // `S::Component` being a generic `serde_json::Value`, which
    // doesn't implement `Hash` or `Ord` but does implement `Eq`.
    component_counts: LinearMap<S::Component, usize>,
}

// Note: have to be careful with lifetimes here because 'a is an
// invariant lifetime. If there are compile errors related to this,
// they're likely to be because 'a got mixed up with a covariant
...
impl<'engine, 'a, S: StepSpec> ComponentRegistrar<'engine, 'a, S> {
    ...
    /// Adds a new step corresponding to the component associated with
    /// the registrar.
    ///
    /// # Notes
    ///
        ...
        T: Send + 'a,
    {
        ...
        let exec_fn = Box::new(move |cx: StepContext<S>| {
            ...
            async move {
                match result.await {
                    Ok(val) => {
                        // Ignore errors if the receiver (the
                        // StepHandle) was dropped.
...
/// 2. Metadata about the step, generated in an async function. For
///    example, this can be a hash of an artifact, or an address it
///    was downloaded from.
...
impl<S: StepSpec> StepExec<'_, S> {
    ...
    > {
        ...
        loop {
            // This is the main execution select loop. We break it
            // up into two portions:
            //
            // 1. The inner select, which is the meat of the engine.
            //    It consists of driving the step and the payload
            //    receiver forward.
            //
            // 2. The outer select, which consists of selecting over
            //    the inner select and the abort receiver.
            //
            // The two selects cannot be combined! That's because
            // the else block of the inner select only applies to
            // the step and payload receivers. We do not want to
            // wait for the abort receiver to exit before exiting
            // the loop.
            let inner_select = async {
                tokio::select! {
                    ...
                    // Note: payload_receiver is always kept open
                    // while step_fut is being driven. It is only
                    // dropped before completion if the step is
                    // aborted, in which case step_fut is also
                    // cancelled without being driven further. A
                    // bunch of expects with "our code always keeps
                    // payload_receiver open" rely on this.
                    //
                    // If we ever move the payload receiver to
                    // another task so it runs in parallel, this
                    // situation would have to be handled with care.
        ...
        // Return the result -- the caller is responsible for handling
        // events.
...
/// NOTE: Ideally this would take `&mut StepContext<S>`, so that it
/// can't get squirreled away by a step's function. However, that
/// quickly runs into [this issue in
/// Rust](https://users.rust-lang.org/t/passing-self-to-callback-returning-future-vs-lifetimes/53352).
///
/// It is probably possible to use unsafe code here, though that opens
...
impl<S: StepSpec, F: FnMut() -> usize> StepProgressReporter<S, F> {
    fn new(step_exec_cx: StepExecutionContext<S, F>) -> Self {
        ...
        Self {
            ...
            step_start,
            attempt: 1,
            // It's slightly nicer for step_start and attempt_start
    ...
    async fn handle_payload(
        ...
        match payload {
            StepContextPayload::Progress { now, progress, done } => {
                ...
                // Dropping the sender signals the receiver that processing is
                // complete.
                drop(done);
            }
    ...
    async fn handle_progress(
        ...
        match progress {
            ...
            StepProgress::Reset { metadata, message } => {
                // Send a progress reset message, but do not reset
                // the attempt.
                self.sender
                    .send(Event::Step(StepEvent {
                        spec: S::spec_name(),
    ...
    async fn handle_abort(mut self, message: String) -> ExecutionError<S> {
        ...
        //
        // The only way this can fail is if the event receiver is
        // closed or dropped. That failure doesn't have any
        // implications on whether this aborts or not.
        let res = self
            .sender
            .send(Event::Step(StepEvent {
                spec: S::spec_name(),
    ...
    async fn send_error(
        ...
        // Stringify `error` into a message + list causes; this is
        // written the way it is to avoid `error` potentially living
        // across the `.await` below (which can cause lifetime issues
        // in callers).
        let (message, causes) = {
...
mod tests {
    use super::*;
    use anyhow::bail;
    use oxide_update_engine_test_utils::TestSpec;
    ...
    async fn error_exits_early() {
        ...
        let engine: UpdateEngine<TestSpec> = UpdateEngine::new(&log, sender);
