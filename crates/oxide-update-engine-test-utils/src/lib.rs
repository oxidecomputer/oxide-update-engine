...
pub enum GenerateTestEventsKind {
    Completed,
    Failed,
    Aborted,
}

pub async fn generate_test_events(
    log: &slog::Logger,
    kind: GenerateTestEventsKind,
) -> Vec<Event<TestSpec>> {
    let (sender, receiver) = oxide_update_engine::channel();
    ...
    match kind {
        ...
        GenerateTestEventsKind::Aborted => {
            // In this case, the last step signals that it has been
            // reached via sending a message over this channel, and
            // then waits forever. We abort execution by calling into
            // the AbortHandle.
            ...
            loop {
                tokio::select! {
                    ...
                    _ = &mut receiver, if !receiver_done => {
                        ...
                        abort_handle
                            .abort(
                                "test engine deliberately aborted"
                            )
                            .expect(
...
fn define_test_steps(
    ...
    engine
        .new_step(
            ...
            async move |parent_cx| {
                ...
                // Define a second nested engine -- this verifies
                // that internal buffer indexes match up.
                parent_cx
                    .with_nested_engine(|engine| {
                        define_nested_engine(
                            ...
                            engine,
                            10,
                            // The tests in buffer.rs expect the
                            // units to be "steps" exactly once,
                            // so use a different name here.
                            "steps (again)",
                        );
                        Ok(())
                    })
                ...
                loop {
                    tokio::select! {
                        res = &mut execute_fut, if !execute_done => {
                            res.expect(
                                "remote nested engine \
                                 completed successfully"
                            );
                            execute_done = true;
                        }
                        Some(event) = receiver.recv() => {
                            // Generate complete reports to ensure
                            // deduping happens within StepContexts.
                            buffer.add_event(event);
                            cx.send_nested_report(
    ...
    // The step index here (100) is large enough to be higher than all
    // nested steps.
    engine
        .new_step("baz".to_owned(), 100, "Step 5", async move |_cx| {
            match last_step_outcome {
                ...
                LastStepOutcome::Aborted(sender) => {
                    ...
                    // The driver of the engine is responsible for
                    // aborting it at this point.
...
pub fn define_remote_nested_engine(
