...
use crate::{
    LineDisplayStyles,
    line_display_shared::{
        HEADER_WIDTH, LineDisplayFormatter, LineDisplayOutput,
        LineDisplayShared,
    },
...
use oxide_update_engine_types::{
    buffer::{
        EventBuffer, ExecutionStatus, ExecutionTerminalInfo, TerminalKind,
    },
...
mod tests {
    ...
    use oxide_update_engine_test_utils::{
        GenerateTestEventsKind, generate_test_events,
    };
    use oxide_update_engine_types::{buffer::EventBuffer, events::EventReport};

    #[tokio::test]
    async fn test_stats() {
        let log = slog::Logger::root(slog::Discard, slog::o!());
        // Generate three sets of events, one for each kind.
        let generated_completed =
            generate_test_events(&log, GenerateTestEventsKind::Completed).await;
        let generated_failed =
            generate_test_events(&log, GenerateTestEventsKind::Failed).await;
        let generated_aborted =
            generate_test_events(&log, GenerateTestEventsKind::Aborted).await;
