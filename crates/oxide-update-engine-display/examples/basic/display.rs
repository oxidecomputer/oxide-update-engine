        ...
        tokio::select! {
            ...
            event = receiver.recv() => {
                ...
                display.add_event_report(
                    ...
                    example_buffer.generate_report_since(
                        &mut example_buffer_last_seen,
                    ),
                )?;
                display.add_event_report(
                    &GroupDisplayKey::Other,
                    other_buffer.generate_report_since(
    ...
    fn finish(
        ...
        match outcome {
            ...
            StepOutcome::Skipped { message, .. } => {
                // Hide the progress bar for skipped steps: show the message.
                self.pb.set_style(
