...
use spec::{
    ...
    ExampleWriteStepId, StepContext, StepHandle, StepProgress, StepSkipped,
    StepSuccess, StepWarning, UpdateEngine,
};
...
/// Creates a slog logger that writes to stderr.
fn setup_log(name: &str) -> slog::Logger {
    use slog::Drain;
    ...
    slog::Logger::root(drain, slog::o!("example" => name.to_owned()))
}
...
impl App {
    async fn exec(self) -> Result<()> {
        let log = setup_log("update_engine_basic_example");
        ...
        let context = ExampleContext::new(&log);
        let (display_handle, sender) =
            make_displayer(&log, display_style, self.prefix);
        ...
        .new_step(
            ...
            async move |cx| {
                ...
                while buf_list.has_remaining() {
                    ...
                    // Don't actually write these bytes -- this engine is
                    // for demoing.
                    let written_bytes =
