...
Typify will generate this struct, but should use the real `EventReport` type (and its transitive dependencies) for the `report` field via `x-rust-type` replacement.*/
///
/// <details><summary>JSON schema</summary>
///
/// ```json
///{
///  "title": "Wrapper",
///  "description": "A wrapper struct that references `EventReport<GenericSpec>`.\n\nTypify will generate this struct, but should use the real `EventReport` type (and its transitive dependencies) for the `report` field via `x-rust-type` replacement.",
///  "type": "object",
///  "required": [
///    "report"
///  ],
///  "properties": {
///    "report": {
///      "$ref": "#/definitions/EventReportForGenericSpec"
///    }
///  }
///}
/// ```
